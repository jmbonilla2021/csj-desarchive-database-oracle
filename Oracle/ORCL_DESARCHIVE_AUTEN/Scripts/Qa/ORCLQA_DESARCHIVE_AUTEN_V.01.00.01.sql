...
BEGIN
    ...
    EXECUTE IMMEDIATE 'CREATE ROLE ROL_ORCLQA_DESARCHIVE_AUTEN';
    PROC_AUDITA_OBJETOS(NULL,NULL,'CREA ROL OK.',NULL,NULL);
    EXECUTE IMMEDIATE 'GRANT CREATE SESSION, CREATE TABLE, CREATE SEQUENCE, CREATE VIEW, CREATE TRIGGER,CREATE PROCEDURE TO ROL_ORCLQA_DESARCHIVE_AUTEN';
    ...
    EXECUTE IMMEDIATE 'GRANT ROL_ORCLQA_DESARCHIVE_AUTEN TO '||V_USUARIO;
...
CREATE TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA (
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LAS APLICACIONES EXTERNAS.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA.ID_APP_EXTERNA IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DE LA APLICACIÓN INTERNA.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA.CODIGO IS
    'CAMPO QUE ALMACENA EL CÓDIGO DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA.HOST IS
    'CAMPO QUE ALMACENA EL HOST (DIRECCIÓN WEB) DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA.PORT IS
    'CAMPO QUE ALMACENA EL PUERTO DEL HOST (DIRECCIÓN WEB) DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA.USERNAME IS
    'CAMPO QUE ALMACENA EL NOMBRE DEL USUARIO DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA.PASSWORD IS
    'CAMPO QUE ALMACENA LA CONTRASEÑA DEL USUARIO DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA.DETALLE IS
    'CAMPO QUE ALMACENA EL DETALLE O LA DESCRIPCIÓN DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA.ES_ACTIVO IS
    'CAMPO QUE ALMACENA EL ESTADO ACTIVO O INACTIVO DE LA APLICACIÓN EXTERNA.';

CREATE UNIQUE INDEX SCH_ORCLQA_DESARCHIVE_AUTEN.IDX_APP_EXTERNA ON
    SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA (
        CODIGO
    ASC );

ALTER TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA ADD CONSTRAINT PK_APE PRIMARY KEY ( ID_APP_EXTERNA );
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.CARGO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS CARGOS.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CARGO.ID_CARGO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DEL CARGO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CARGO.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DEL CARGO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CARGO.ES_ACTIVO IS
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.CREDENCIAL IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LAS CREDENCIALES DE LOS USUARIOS.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CREDENCIAL.ID_CREDENCIAL IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DE CREDENCIAL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CREDENCIAL.USUARIO IS
    'CAMPO QUE ALMACENA EL USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CREDENCIAL.CONTRASENA IS
    'CAMPO QUE ALMACENA LA CONTRASEÑA.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CREDENCIAL.INTENTOS_INVALIDOS IS
    'CAMPO QUE ALMACENA LOS INTENTOS INVÁLIDOS DE INGRESO A LA APLICACIÓN.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CREDENCIAL.FECHA_BLOQUEO IS
    'CAMPO QUE ALMACENA LA FECHA DE BLOQUEO DEL USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CREDENCIAL.ES_BLOQUEADO IS
    'CAMPO QUE ALMACENA EL ESTADO BLOQUEADO O NO BLOQUEADO DEL USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.CREDENCIAL.ID_USUARIO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DEL USUARIO.';

CREATE UNIQUE INDEX SCH_ORCLQA_DESARCHIVE_AUTEN.IDX_CREDENCIAL ON
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.DEPARTAMENTO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS DEPARTAMENTOS.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.DEPARTAMENTO.ID_DEPARTAMENTO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DEL DEPARTAMENTO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.DEPARTAMENTO.CODIGO IS
    'CAMPO QUE ALMACENA EL CÓDIGO ÚNICO POR CADA DEPARTAMENTO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.DEPARTAMENTO.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DEL DEPARTAMENTO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.DEPARTAMENTO.ES_ACTIVO IS
    'CAMPO QUE ALMACENA EL ESTADO ACTIVO O INACTIVO DEL DEPARTAMENTO.';

CREATE UNIQUE INDEX SCH_ORCLQA_DESARCHIVE_AUTEN.IDX_DEPARTAMENTO ON
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.DETALLE_USUARIO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DEL DETALLE DE LOS USUARIOS.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.DETALLE_USUARIO.ID_DETALLE_USUARIO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DEL DETALLE DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.DETALLE_USUARIO.ID_USUARIO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DEL USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.DETALLE_USUARIO.ID_CARGO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DEL CARGO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.DETALLE_USUARIO.ID_MUNICIPIO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DEL MUNICIPIO.';

CREATE UNIQUE INDEX SCH_ORCLQA_DESARCHIVE_AUTEN.IDX_DETALLE_USUARIO ON
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.GRUPO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS GRUPOS DE ROLES DE USUARIO DE LA APLICACIÓN.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.GRUPO.ID_GRUPO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DEL GRUPO DE ROLES DE USUARIO DE LA APLICACIÓN.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.GRUPO.CODIGO IS
    'CAMPO QUE ALMACENA EL CÓDIGO DEL GRUPO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.GRUPO.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DEL GRUPO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.GRUPO.ES_ACTIVO IS
    'CAMPO QUE ALMACENA EL ESTADO ACTIVO O INACTIVO DEL GRUPO.';

CREATE UNIQUE INDEX SCH_ORCLQA_DESARCHIVE_AUTEN.IDX_GRUPO ON
    SCH_ORCLQA_DESARCHIVE_AUTEN.GRUPO (
        CODIGO
    ASC );
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.MENU IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS MENÚS.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.MENU.ID_MENU IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DE MENÚ.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.MENU.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DEL MENÚ.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.MENU.ID_MENU_PADRE IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DEL MENÚ PADRE.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.MENU.URL IS
    'CAMPO QUE ALMACENA LA DIRECCIÓN URL DEL MENÚ.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.MENU.ES_ACTIVO IS
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.MENU_X_ROL IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS MENÚS POR ROLES (PERFILES) DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.MENU_X_ROL.ID_MENU IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DE MENÚ.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.MENU_X_ROL.ID_ROL IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DE ROL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.MENU_X_ROL.ID_PERMISOSXMODULO IS
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.PARAMETRO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS PARÁMETROS UTILIZADOS EN LA APLICACIÓN.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PARAMETRO.ID_PARAMETRO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DEL PARÁMETRO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PARAMETRO.NOMBRE_CLAVE IS
    'CAMPO QUE ALMACENA EL NOMBRE CLAVE DEL PARÁMETRO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PARAMETRO.VALOR IS
    'CAMPO QUE ALMACENA EL VALOR DEL PARÁMETRO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PARAMETRO.ID_PROCEDIMIENTO IS
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOS_X_MODULO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS PERMISOS POR MÓDULO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOS_X_MODULO.ID_PERMISOSXMODULO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DEL PERMISO POR ROL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOS_X_MODULO.CONSULTAR IS
    'CAMPO QUE INDICA SI EL PERMISO DE CONSULTAR ESTÁ ACTIVO PARA EL MÓDULO CORRESPONDIENTE.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOS_X_MODULO.CREAR IS
    'CAMPO QUE INDICA SI EL PERMISO DE CREAR ESTÁ ACTIVO PARA EL MÓDULO CORRESPONDIENTE.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOS_X_MODULO.EDITAR IS
    'CAMPO QUE INDICA SI EL PERMISO DE EDITAR ESTÁ ACTIVO PARA EL MÓDULO CORRESPONDIENTE.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOS_X_MODULO.ES_ACTIVO IS
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOSXMODULOXROL IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS PERMISOS POR MÓDULO POR ROL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOSXMODULOXROL.ID_PERMXMODXROL IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DE PERMISO POR MÓDULO POR ROL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOSXMODULOXROL.ID_MENU IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DE MENÚ.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOSXMODULOXROL.ID_ROL IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DE ROL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOSXMODULOXROL.CONSULTAR IS
    'CAMPO QUE INDICA SI EL ROL TIENE PERMISO DE CONSULTAR EN EL MENÚ CORRESPONDIENTE.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOSXMODULOXROL.CREAR IS
    'CAMPO QUE INDICA SI EL ROL TIENE PERMISO DE CREAR EN EL MENÚ CORRESPONDIENTE.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOSXMODULOXROL.EDITAR IS
    'CAMPO QUE INDICA SI EL ROL TIENE PERMISO DE EDITAR EN EL MENÚ CORRESPONDIENTE.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOSXMODULOXROL.ES_ACTIVO IS
    'CAMPO QUE ALMACENA EL ESTADO ACTIVO O INACTIVO DEL PERMISO.';

ALTER TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.PERMISOSXMODULOXROL ADD CONSTRAINT PK_PMR PRIMARY KEY ( ID_PERMXMODXROL );

CREATE TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO (
    ID_PROCEDIMIENTO NUMBER(38) NOT NULL,
    CODIGO           NVARCHAR2(1000) NOT NULL,
    NOMBRE           NVARCHAR2(1000) NOT NULL,
    ES_ACTIVO        NUMBER(1) NOT NULL
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS PROCEDIMIENTOS O SISTEMAS QUE TIENEN PARÁMETROS PARA SU FUNCIONAMIENTO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO.ID_PROCEDIMIENTO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DE PROCEDIMIENTO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO.CODIGO IS
    'CAMPO QUE ALMACENA EL CÓDIGO ÚNICO POR CADA PROCEDIMIENTO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DEL PROCEDIMIENTO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO.ES_ACTIVO IS
    'CAMPO QUE ALMACENA EL ESTADO ACTIVO O INACTIVO DEL PROCEDIMENTO.';

CREATE UNIQUE INDEX SCH_ORCLQA_DESARCHIVE_AUTEN.IDX_PROCEDIMIENTO ON
    SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO (
        CODIGO
    ASC );

ALTER TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO ADD CONSTRAINT PK_PRC PRIMARY KEY ( ID_PROCEDIMIENTO );
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.ROL IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS ROLES (PERFILES) DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.ROL.ID_ROL IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DE ROL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.ROL.CODIGO IS
    'CAMPO QUE ALMACENA EL CÓDIGO DE ROL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.ROL.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DE ROL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.ROL.ES_ACTIVO IS
    'CAMPO QUE ALMACENA EL ESTADO ACTIVO O INACTIVO DE ROL.';

CREATE UNIQUE INDEX SCH_ORCLQA_DESARCHIVE_AUTEN.IDX_ROL ON
    SCH_ORCLQA_DESARCHIVE_AUTEN.ROL (
        CODIGO
    ASC );
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.ROLESXGRUPO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS ROLES (PERFILES) POR GRUPO DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.ROLESXGRUPO.ID_ROL IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DE ROL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.ROLESXGRUPO.ID_GRUPO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DE GRUPO';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.ROLESXGRUPO.ES_ACTIVO IS
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.SECCIONAL IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LAS SECCIONALES.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.SECCIONAL.ID_SECCIONAL IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DE LA SECCIONAL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.SECCIONAL.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DE LA SECCIONAL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.SECCIONAL.ES_ACTIVO IS
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.TIPO_IDENTIFICACION IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS TIPO DE IDENTIFICACIÓN.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.TIPO_IDENTIFICACION.ID_TIPO_IDENTIFICACION IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DE TIPO DE IDENTIFICACIÓN.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.TIPO_IDENTIFICACION.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DEL TIPO DE IDENTIFICACIÓN.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.TIPO_IDENTIFICACION.ABREVIACION IS
    'CAMPO QUE ALMACENA LA ABREVIACIÓN DEL TIPO DE IDENTIFICACIÓN.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.TIPO_IDENTIFICACION.ES_ACTIVO IS
...
COMMENT ON TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS USUARIOS.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.ID_USUARIO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DEL USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.NUMERO_IDENTIFICACION IS
    'CAMPO QUE ALMACENA EL NÚMERO DE IDENTIFICACIÓN DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.PRIMER_NOMBRE IS
    'CAMPO QUE ALMACENA EL PRIMER NOMBRE DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.SEGUNDO_NOMBRE IS
    'CAMPO QUE ALMACENA EL SEGUNDO NOMBRE DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.PRIMER_APELLIDO IS
    'CAMPO QUE ALMACENA EL PRIMER APELLIDO DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.SEGUNDO_APELLIDO IS
    'CAMPO QUE ALMACENA EL SEGUNDO APELLIDO DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.EMAIL IS
    'CAMPO QUE ALMACENA EL CORREO ELECTRÓNICO DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.RAZON_SOCIAL IS
    'CAMPO QUE ALMACENA LA RAZÓN SOCIAL DEL USUARIO (PJ).';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.ES_GESTOR_PROCESAL IS
    'CAMPO QUE ALMACENA SI EL USUARIO ES O NO GESTOR PROCESAL.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.ES_ACTIVO IS
    'CAMPO QUE ALMACENA EL ESTADO ACTIVO O INACTIVO DE USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.ID_ROL IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR DE ROL AL CUAL PERTENECE EL USUARIO.';

COMMENT ON COLUMN SCH_ORCLQA_DESARCHIVE_AUTEN.USUARIO.ID_TIPO_IDENTIFICACION IS
...
ALTER TABLE SCH_ORCLQA_DESARCHIVE_AUTEN.PARAMETRO
    ADD CONSTRAINT FK_PRC_PRM FOREIGN KEY ( ID_PROCEDIMIENTO )
        REFERENCES SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO ( ID_PROCEDIMIENTO );
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_APPS_INTERNAS START WITH 1 CACHE 20 ORDER;

CREATE OR REPLACE TRIGGER SCH_ORCLQA_DESARCHIVE_AUTEN.TRG_APPS_INTERNAS BEFORE
    INSERT ON SCH_ORCLQA_DESARCHIVE_AUTEN.APP_EXTERNA
    FOR EACH ROW
    WHEN ( NEW.ID_APP_EXTERNA IS NULL )
BEGIN
    :NEW.ID_APP_EXTERNA := SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_APPS_INTERNAS.NEXTVAL;
END;
/

CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_CARGO START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_CREDENCIAL START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_DEPARTAMENTO START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_DETALLE_USUARIO START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_GRUPO START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_MENU START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_MUNICIPIO START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_PARAMETRO START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_PERMISOSXMODULO START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_PERMISOSXMODULOXROL START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_PROCEDIMIENTO START WITH 1 CACHE 20 ORDER;

CREATE OR REPLACE TRIGGER SCH_ORCLQA_DESARCHIVE_AUTEN.TRG_PROCEDIMIENTO BEFORE
    INSERT ON SCH_ORCLQA_DESARCHIVE_AUTEN.PROCEDIMIENTO
    FOR EACH ROW
    WHEN ( NEW.ID_PROCEDIMIENTO IS NULL )
BEGIN
    :NEW.ID_PROCEDIMIENTO := SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_PROCEDIMIENTO.NEXTVAL;
END;
/

CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_ROL START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_SECCIONAL START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_TIPO_IDENTIFICACION START WITH 1 CACHE 20 ORDER;
...
CREATE SEQUENCE SCH_ORCLQA_DESARCHIVE_AUTEN.SEQ_USUARIO START WITH 1 CACHE 20 ORDER;
...
DECLARE
  ...
  IS 
	...
	FROM   ALL_TABLES
    WHERE  OWNER = 'SCH_ORCLQA_DESARCHIVE_AUTEN';
...
BEGIN
  ...
  V_ROL := 'ROL_ORCLQA_DESARCHIVE_AUTEN';
  OPEN C_PERMISOS;
  LOOP
