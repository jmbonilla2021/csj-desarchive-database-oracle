...
CREATE TABLE SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA (
...
COMMENT ON TABLE SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LAS APLICACIONES EXTERNAS.';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA.ID_APP_EXTERNA IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DE LA APLICACIÓN INTERNA.';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA.NOMBRE IS
    'CAMPO QUE ALMACENA EL NOMBRE DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA.CODIGO IS
    'CAMPO QUE ALMACENA EL CÓDIGO DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA.HOST IS
    'CAMPO QUE ALMACENA EL HOST (DIRECCIÓN WEB) DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA.PORT IS
    'CAMPO QUE ALMACENA EL PUERTO DEL HOST (DIRECCIÓN WEB) DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA.USERNAME IS
    'CAMPO QUE ALMACENA EL NOMBRE DEL USUARIO DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA.PASSWORD IS
    'CAMPO QUE ALMACENA LA CONTRASEÑA DEL USUARIO DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA.DETALLE IS
    'CAMPO QUE ALMACENA EL DETALLE O LA DESCRIPCIÓN DE LA APLICACIÓN EXTERNA.';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA.ES_ACTIVO IS
    'CAMPO QUE ALMACENA EL ESTADO ACTIVO O INACTIVO DE LA APLICACIÓN EXTERNA.';

CREATE UNIQUE INDEX SCH_ORCL_DESARCHIVE_AUTEN_PRE.IDX_APP_EXTERNA ON
    SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA (
        CODIGO
    ASC );

ALTER TABLE SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA ADD CONSTRAINT PK_APE PRIMARY KEY ( ID_APP_EXTERNA );
...
COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.GRUPO.CODIGO IS
...
CREATE UNIQUE INDEX SCH_ORCL_DESARCHIVE_AUTEN_PRE.IDX_GRUPO ON
    SCH_ORCL_DESARCHIVE_AUTEN_PRE.GRUPO (
        CODIGO
    ASC );
...
COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.ROL.CODIGO IS
...
CREATE UNIQUE INDEX SCH_ORCL_DESARCHIVE_AUTEN_PRE.IDX_ROL ON
    SCH_ORCL_DESARCHIVE_AUTEN_PRE.ROL (
        CODIGO
    ASC );
...
COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.USUARIO.RAZON_SOCIAL IS
    'CAMPO QUE ALMACENA LA RAZÓN SOCIAL DEL USUARIO (PJ).';

COMMENT ON COLUMN SCH_ORCL_DESARCHIVE_AUTEN_PRE.USUARIO.ES_GESTOR_PROCESAL IS
...
CREATE OR REPLACE TRIGGER SCH_ORCL_DESARCHIVE_AUTEN_PRE.TRG_APPS_INTERNAS BEFORE
    INSERT ON SCH_ORCL_DESARCHIVE_AUTEN_PRE.APP_EXTERNA
    FOR EACH ROW
    WHEN ( NEW.ID_APP_EXTERNA IS NULL )
BEGIN
    :NEW.ID_APP_EXTERNA := SCH_ORCL_DESARCHIVE_AUTEN_PRE.SEQ_APPS_INTERNAS.NEXTVAL;
END;
/
...
CREATE SEQUENCE SCH_ORCL_DESARCHIVE_AUTEN_PRE.SEQ_PROCEDIMIENTO START WITH 1 CACHE 20 ORDER;

CREATE OR REPLACE TRIGGER SCH_ORCL_DESARCHIVE_AUTEN_PRE.TRG_PROCEDIMIENTO BEFORE
    INSERT ON SCH_ORCL_DESARCHIVE_AUTEN_PRE.PROCEDIMIENTO
    FOR EACH ROW
    WHEN ( NEW.ID_PROCEDIMIENTO IS NULL )
BEGIN
    :NEW.ID_PROCEDIMIENTO := SCH_ORCL_DESARCHIVE_AUTEN_PRE.SEQ_PROCEDIMIENTO.NEXTVAL;
END;
/
