...
DECLARE
   ...
   IS
     ...
     INTO   v_sesion
     FROM   V$SESSION VS
     WHERE  USERNAME = 'SCH_ORCL_DESARCHIVE_CORE';
...
BEGIN
	DBMS_SERVICE.stop_service('ORCL_DESARCHIVE_CORE');
END;
/
BEGIN
	DBMS_SERVICE.delete_service('ORCL_DESARCHIVE_CORE');
END;
/


DROP ROLE ROL_DESARCHIVE_CORE;
DROP USER SCH_ORCL_DESARCHIVE_CORE CASCADE;

DROP TABLESPACE ORCL_DESARCHIVE_CORE_DATA INCLUDING CONTENTS;
DROP TABLESPACE ORCL_DESARCHIVE_CORE_IDX  INCLUDING CONTENTS;
