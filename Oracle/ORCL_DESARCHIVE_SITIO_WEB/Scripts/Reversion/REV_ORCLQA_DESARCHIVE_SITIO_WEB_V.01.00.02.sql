/*
-------------------------------------------------------------------------------------------------------------------------------
-- OBJETIVO                  : Script de reversion del script ORCLQA_DESARCHIVE_SITIO_WEB_01_00_02.sql
...
DECLARE
   ...
   IS
     ...
     INTO   v_sesion
     FROM   V$SESSION VS
     WHERE  USERNAME = 'SCH_ORCLQA_DESARCHIVE_SWEB';
...
BEGIN
	DBMS_SERVICE.stop_service('ORCLQA_DESARCHIVE_SWEB');
END;
/
BEGIN
	DBMS_SERVICE.delete_service('ORCLQA_DESARCHIVE_SWEB');
END;
/

DROP ROLE ROL_ORCLQA_DESARCHIVE_SWEB;
DROP USER SCH_ORCLQA_DESARCHIVE_SWEB CASCADE;

DROP TABLESPACE ORCLQA_DESARCHIVE_SWEB_DATA INCLUDING CONTENTS;
DROP TABLESPACE ORCLQA_DESARCHIVE_SWEB_IDX  INCLUDING CONTENTS;