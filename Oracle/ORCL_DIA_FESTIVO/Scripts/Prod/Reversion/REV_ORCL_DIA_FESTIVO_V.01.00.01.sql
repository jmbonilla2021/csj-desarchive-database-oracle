...
DECLARE
   ...
   IS
     ...
     INTO   v_sesion
     FROM   V$SESSION VS
     WHERE  USERNAME = 'SCH_ORCL_DIA_FESTIVO';
...
BEGIN
	DBMS_SERVICE.stop_service('ORCL_DIA_FESTIVO');
END;
/
BEGIN
	DBMS_SERVICE.delete_service('ORCL_DIA_FESTIVO');
END;
/


DROP ROLE ROL_DIA_FESTIVO;
DROP USER SCH_ORCL_DIA_FESTIVO CASCADE;

DROP TABLESPACE ORCL_DIA_FESTIVO_DATA INCLUDING CONTENTS;
DROP TABLESPACE ORCL_DIA_FESTIVO_IDX  INCLUDING CONTENTS;
