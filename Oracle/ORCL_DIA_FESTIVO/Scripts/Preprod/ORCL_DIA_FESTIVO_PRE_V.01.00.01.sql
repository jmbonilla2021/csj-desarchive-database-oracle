...
BEGIN
	...
	PROC_AUDITA_OBJETOS('CSJ','ORCL_DIA_FESTIVO_PRE','CREACION BD',NULL,NULL);
    V_SERVICIO := '&SERVICIO';
    ...
    EXECUTE IMMEDIATE 'CREATE ROLE ROL_DIA_FESTIVO_PRE';
    PROC_AUDITA_OBJETOS(NULL,NULL,'CREA ROL OK.',NULL,NULL);
    EXECUTE IMMEDIATE 'GRANT CREATE SESSION, CREATE TABLE, CREATE SEQUENCE, CREATE VIEW, CREATE TRIGGER,CREATE PROCEDURE TO ROL_DIA_FESTIVO_PRE';
    ...
    EXECUTE IMMEDIATE 'GRANT ROL_DIA_FESTIVO_PRE TO '||V_USUARIO;
...
CREATE TABLE SCH_ORCL_DIA_FESTIVO_PRE.DIA_FESTIVO (
...
COMMENT ON TABLE SCH_ORCL_DIA_FESTIVO_PRE.DIA_FESTIVO IS
    'TABLA QUE ALMACENA LA INFORMACIÓN DE LOS DÍAS FESTIVOS EN EL CALENDARIO COLOMBIANO.';

COMMENT ON COLUMN SCH_ORCL_DIA_FESTIVO_PRE.DIA_FESTIVO.ID_DIA_FESTIVO IS
    'CAMPO QUE ALMACENA EL IDENTIFICADOR ÚNICO DEL DÍA FESTIVO.';

COMMENT ON COLUMN SCH_ORCL_DIA_FESTIVO_PRE.DIA_FESTIVO.ANIO IS
    'CAMPO QUE ALMACENA EL AÑO DEL DÍA FESTIVO.';

COMMENT ON COLUMN SCH_ORCL_DIA_FESTIVO_PRE.DIA_FESTIVO.MES IS
    'CAMPO QUE ALMACENA EL MES DEL DÍA FESTIVO.';

COMMENT ON COLUMN SCH_ORCL_DIA_FESTIVO_PRE.DIA_FESTIVO.DIA IS
    'CAMPO QUE ALMACENA EL DÍA FESTIVO DENTRO DEL MES Y AÑO CORRESPONDIENTE.';

COMMENT ON COLUMN SCH_ORCL_DIA_FESTIVO_PRE.DIA_FESTIVO.FECHA IS
    'CAMPO QUE ALMACENA EL DÍA FESTIVO EN FORMATO FECHA.';

ALTER TABLE SCH_ORCL_DIA_FESTIVO_PRE.DIA_FESTIVO ADD CONSTRAINT PK_DFE PRIMARY KEY ( ID_DIA_FESTIVO );

CREATE SEQUENCE SCH_ORCL_DIA_FESTIVO_PRE.SEQ_DIA_FESTIVO START WITH 1 CACHE 20 ORDER;

CREATE OR REPLACE TRIGGER SCH_ORCL_DIA_FESTIVO_PRE.TRG_DIA_FESTIVO BEFORE
    INSERT ON SCH_ORCL_DIA_FESTIVO_PRE.DIA_FESTIVO
    FOR EACH ROW
    WHEN ( NEW.ID_DIA_FESTIVO IS NULL )
BEGIN
    :NEW.ID_DIA_FESTIVO := SCH_ORCL_DIA_FESTIVO_PRE.SEQ_DIA_FESTIVO.NEXTVAL;
END;
/
...
DECLARE
  ...
  CURSOR C_PERMISOS
  IS 
    SELECT 'GRANT SELECT, INSERT, UPDATE, DELETE ON SCH_ORCL_DIA_FESTIVO_PRE.'||TABLE_NAME||' TO ' AS SENTENCIA
	FROM   ALL_TABLES
    WHERE  OWNER = 'SCH_ORCL_DIA_FESTIVO_PRE';
...
DECLARE
  ...
  V_IDX   VARCHAR2(4000):= 'ORCL_DIA_FESTIVO_PRE_IDX';
  V_OW    VARCHAR2(4000):= 'SCH_ORCL_DIA_FESTIVO_PRE';
...
BEGIN
  ...
  PROC_AUDITA_OBJETOS('CSJ','ORCL_DIA_FESTIVO_PRE','FINALIZACION CREACION BD',NULL,NULL);
EXCEPTION
  WHEN OTHERS THEN
